...
- Coming to the main part, which is Verification type, projects are provided with 3 options as mentioned below:

  - **Collect Verified Email**: This task type allows you to collect verified email addresses from participants. Entries will be rewarded to every participant who enters their email and verifies.

  - **Whitelist Email**: Upon selecting this task type, you will be provided to upload or drag a CSV file that contains all the email addresses. You can download a sample file from the download link below and enter the required email addresses in the CSV file for upload. Based on the CSV file uploaded, AirLyft's internal verification algorithm would check if the participant's email address is available in the list or not and then reward entries.

  - **Subscribe Newsletter**: This option allows you to grow your newsletter subscriber base using various email marketing providers like Mailchimp, Brevo (Sendinblue), MailerLite, and others. You will need to provide credentials for your chosen email service provider to enable this integration.

  ![Email upload](../../../images/EmailUpload.png)

- Finally click on Add Task button and that's it. You have created your Email task successfully.
